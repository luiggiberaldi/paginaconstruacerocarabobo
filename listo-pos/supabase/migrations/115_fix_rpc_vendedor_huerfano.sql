-- 115_fix_rpc_vendedor_huerfano.sql
-- Actualiza el RPC para manejar la búsqueda de vendedores no asignados mediante un UUID de ceros.
...
BEGIN
  ...
  items_con_comision AS (
    ...
    WHERE (
      p_vendedor_id IS NULL -- Todos
      OR (p_vendedor_id = v_uuid_nulo AND COALESCE(cl.vendedor_id, nd.vendedor_id) IS NULL) -- Huérfanos
      OR (COALESCE(cl.vendedor_id, nd.vendedor_id) = p_vendedor_id) -- Vendedor específico
    )
  )
  SELECT 
