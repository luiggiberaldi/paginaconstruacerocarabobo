-- 099_fix_rpc_total_profundidad.sql
-- RPC para edición profunda de despachos.
-- Recibe la identidad del operador como parámetros explícitos porque el worker
-- usa service_role key y auth.uid() sería null dentro del RPC.
...
BEGIN
  ...
  FOR v_item_json IN SELECT * FROM jsonb_to_recordset(p_nuevos_items) AS x(
    ...
    -- Insertar ítem (cantidad_original = valor al momento de la edición profunda)
  ...
  -- 7. Auditoría (usa parámetros explícitos, no auth.uid())
